...
resource "aws_api_gateway_rest_api" "cloud_gamer" {
  name        = var.project-name
  description = "The API Gateway connected to the ${var.project-name} Lambda functions."
}

# Cloud Gamer resource
resource "aws_api_gateway_resource" "cloud_gamer" {
  rest_api_id = aws_api_gateway_rest_api.cloud_gamer.id
  parent_id   = aws_api_gateway_rest_api.cloud_gamer.root_resource_id
  path_part   = "cloud-gamer"
}
...
resource "aws_api_gateway_resource" "turn_on" {
  rest_api_id = aws_api_gateway_rest_api.cloud_gamer.id
  parent_id   = aws_api_gateway_resource.cloud_gamer.id
  path_part   = "turn-on"
}
...
resource "aws_api_gateway_deployment" "development" {
  ...
  triggers = {
    redeployment = sha1(jsonencode(aws_api_gateway_rest_api.cloud_gamer.body))
  }
...
resource "aws_lambda_permission" "turn_on" {
  ...
  source_arn = "${aws_api_gateway_rest_api.cloud_gamer.execution_arn}/*/*" #FIXME how to tighten this?
}
...
resource "aws_api_gateway_api_key" "cloud_gamer" {
  name = "cloud_gamer"
}

resource "aws_api_gateway_usage_plan" "cloud_gamer" {
  name        = "cloud-gamer-secure-api"
  description = "Restricts access to the API with an API key."
  #product_code = "MYCODE"

  api_stages {
    api_id = aws_api_gateway_rest_api.cloud_gamer.id
...
resource "aws_api_gateway_usage_plan_key" "cloud_gamer" {
  key_id        = aws_api_gateway_api_key.cloud_gamer.id
  key_type      = "API_KEY"
  usage_plan_id = aws_api_gateway_usage_plan.cloud_gamer.id
}
